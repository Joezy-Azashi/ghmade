...
function deactivatedAccount ({currentPage}){
    ...
    const [title, setTitle] = useState("Deactivated Accounts");
    ...
    //   useEffect(() => {
    //     (async () => {
    //       const orgs = await new Users().activateDeactivateacc(
    //         currentPage
    //       )
    ...
    //   }, [currentPage]);
      ...
      const activateDeactivate = async () => {
        ...
        for (let i = 0; i < checkedUsers.length; i++) {
          let rs = await new Users().activateDeactivateacc({
            pk: checkedUsers[i],
            active,
          });
        }
